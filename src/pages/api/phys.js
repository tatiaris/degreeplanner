...
handler.get(async (req, res) => {
  let courseCategories = {
    ...
    Supporting: {
      courses:
        "(phys-(303|305|401|414|416|425|426|414|416|489|[6-9][0-9][0-9])|astr-(314|[6-9][0-9][0-9]))",
      hours: 7,
    },
    Communication: {
    ...
    Mathematics: {
      courses:
        "(math-(140|142|147|148|150|151|152|167|168|171|172)|phil-(240)|stat-(201))",
      hours: 8,
    },
    ...
    "General Electives": {
      courses:
        "(csce-(110|181|222)|engr-(102)|geog-(201)|hlth-(236)|math-(302|304)|phys-(216)|stat-(211|212))",
      hours: 8,
    },
    Other: {
      courses: "^(?:(?!(",
  ...
  for (let category in courseCategories) {
    if (category != "Other")
      courseCategories.Other.courses +=
        courseCategories[category].courses + "|";
    else
      courseCategories.Other.courses =
        courseCategories.Other.courses.slice(0, -1) + ")))";

    await fetch(
      `${BASE_URL}/api/courses?pattern=${courseCategories[category].courses}`,
      {
        method: "get",
      }
    )
