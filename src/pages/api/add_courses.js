...
handler.get(async (req, res) => {
  ...
  let courses = []
  $(searchContext).each(function(i, elem) {
    let courseHtml = $(this)
    ...
    let courseDescription = courseHtml.find($('.courseblockdesc')).text().trim();
  ...
  for (let i = 0; i < courses.length; i++) {
    let r = await req.db.collection("courses").update({id: courses[i].id}, courses[i], {upsert: true});
